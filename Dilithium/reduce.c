...
int32_t montgomery_reduce(int64_t a) { // 나눗셈 없이 mod Q 연산을 빠르게 계산하는 알고리즘(나눗셈 대신 곱셈과 시프트만으로)
  ...
  t = (int64_t)(int32_t)a*QINV; // a의 하위 32비트만을 사용해서 Q의 역원을 곱해, 몽고메리 상수를 만드는 단계 (a + tQ를 2^32의 배수로 만들고)
  t = (a - (int64_t)t*Q) >> 32; // 상위 32비트 사용 (>> 32 해서 mod Q 결과를 얻는 기법)
  return t;
}
...
* Returns r.
**************************************************/
int32_t caddq(int32_t a) { // 조건부로 +Q 를 더해서 음수를 양수 대표로 바꾸기 (값이 0 ~ q-1 범위)
